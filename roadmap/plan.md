# NeuroSpect: AI Trading Platform — Complete Product & Technical Plan
...
**Components:**
- **NeuroSpect** — Company and product brand
...
- **NeuroLLM** — Underlying LLM/AI technology layer (RAG, fine-tuning, agent orchestration, prompt management)
...
├── NeuroLLM.md                  # Original strategic spec (reference)
│
├── ── KNOWLEDGE ──────────────
...
│   ├── plan.md                  # Active master plan
│   ├── plan-versions/           # Historical plan versions
│   ├── status.md                # Phase status dashboard (updated by /sync)
│   └── phases/phase-N-name/     # Per-phase: README, deviations, boot-prompts/
│
├── ── BACKEND ────────────────
...
│   │   │   ├── rag/             # NEW (Phase 1): NeuroCortex retrieval pipeline
│   │   │   │   ├── embedder.py
│   │   │   │   ├── retriever.py
│   │   │   │   ├── ingestor.py
│   │   │   │   └── evaluator.py
...
│
├── ── FRONTEND ───────────────
├── app/                         # React 19 frontend (extended)
│   ├── src/
│   │   ├── pages/
│   │   │   ├── chat.tsx         # NEW: coaching chat
│   │   │   ├── roadmap.tsx      # NEW: roadmap dashboard
│   │   │   └── ...
│   │   ├── components/
│   │   │   ├── chat/            # NEW: chat UI components
│   │   │   ├── roadmap/         # NEW: roadmap dashboard components
│   │   │   │   ├── PhaseGraph.tsx       # Interactive phase progress graph
│   │   │   │   ├── PhaseNode.tsx        # Clickable phase node
│   │   │   │   ├── FeatureSubGraph.tsx  # Drilldown: features as nodes
│   │   │   │   ├── StatusTimeline.tsx   # Timeline view
│   │   │   │   └── EngineerActivity.tsx # Who's working on what
│   │   │   └── ...
│   │   └── hooks/
│   │       ├── use-chat.ts
│   │       ├── use-roadmap.ts   # NEW: roadmap data fetching
│   │       └── ...
│
├── ── TOOLING ────────────────
├── .claude/skills/              # Claude Code skills
│   ├── phase.md                 # /phase N [plan|exec] — runtime boot prompt
│   ├── sync.md                  # /sync — end-of-session state sync
│   └── crossref.md              # /crossref — cross-wiki intelligence
...
**What is NeuroSpect?** An AI trading platform that turns a trading educator's knowledge, voice, and teaching style into a personalized AI coach available 24/7 to every student — with backtesting, quantitative analysis, and automated trading capabilities.
...
| Category | Key Players | NeuroSpect Differentiation |
|---|---|---|
| AI trading assistants | Price Action Lover, TradingGPT | Single methodology, no personalization, no instructor voice. NeuroSpect has exclusive instructor content + fine-tuned voice. |
...
| TradingView ecosystem | LuxAlgo (800K followers) | Detection tools, not coaching. LuxAlgo shows "here's an OB"; NeuroSpect says "that's not an OB because it didn't precede displacement." |
...
- **NeuroSpect** → The brand, company, and product. Used everywhere — B2B and B2C.
- **NeuroLLM** → Internal name for the AI/LLM technology layer (not customer-facing)
- **Tagline:** "NeuroSpect — Your AI trading coach"
...
│                     SESSION START                                    │
│                                                                      │
│  Engineer opens Claude Code in neurospect/ repo                      │
│                         │                                            │
│                         ▼                                            │
│               ┌─────────────────┐                                    │
│               │  /phase N exec  │  ← Loads runtime boot prompt       │
│               │  or /phase N    │    from roadmap + deviations +     │
│               │  plan           │    Linear + git state              │
│               └────────┬────────┘                                    │
│                        │                                             │
│           ┌────────────┼────────────┐                                │
│           ▼            ▼            ▼                                │
...
│                        ▼                                             │
│               ┌─────────────────┐                                    │
│               │   /crossref     │  ← Optional: check what the        │
│               │   (if needed)   │    other engineer has written       │
│               └────────┬────────┘                                    │
│                        │                                             │
│                        ▼                                             │
│               ┌─────────────────┐                                    │
│               │    /sync        │  ← MANDATORY before session end    │
│               │                 │                                    │
│               │ 1. Linear sync  │    Updates ticket status           │
│               │ 2. Boot regen   │    Regenerates boot prompts        │
│               │ 3. Phase status │    Updates roadmap/status.md       │
│               │ 4. Deviations   │    Captures plan deviations        │
│               │ 5. Cross-wiki   │    Flags relevant other-wiki       │
│               │ 6. Suggest next │    Recommends next /phase skill    │
│               └────────┬────────┘                                    │
│                        │                                             │
│                        ▼                                             │
│               ┌─────────────────┐                                    │
...
1. **Start session** → Run `/phase N` to load context for the phase you're working on
2. **Work** → Code, research, or plan. Write notes in your personal wiki. Edit code in `api/` or `app/`.
3. **Mid-session check** → Run `/crossref` if you want to see what the other engineer has written about your current topic
4. **End session** → Run `/sync` (mandatory). It syncs everything and tells you what to run next.
...
main (always deployable)
  │
  ├── feat/NEU-042-rag-retrieval-pipeline     ← Feature branch per ticket
  ├── feat/NEU-043-chat-api-endpoint
  ├── fix/NEU-055-hallucination-on-order-blocks
  └── research/NEU-015-lora-baseline           ← Research branches don't merge to main
```

- **Branch naming:** `{type}/NEU-{ticket}-{short-description}`
- **PR naming:** `[NEU-042] Add RAG retrieval pipeline`
- **PR body:** Summary + test plan + `Generated with Claude Code`
- **Merge:** Squash merge to main. Render + Cloudflare Pages auto-deploy.
- **After merge:** `/sync` closes the Linear ticket automatically.

### Personal Wiki Workflow

```
Engineer works on NEU-042 (RAG retrieval)
         │
         ▼
Creates: vlad-wiki/phases/phase-1-rag-mvp/tickets/NEU-042-rag-retrieval/
         ├── README.md        ← Implementation notes, decisions, blockers
         └── boot-prompt.md   ← Ticket-specific boot prompt (if multi-session)
         │
         ▼ (during work)
Writes research notes in vlad-wiki/research/chunking-strategies.md
         │
         ▼ (research matures)
Adds `promote: true` to frontmatter
         │
         ▼ (/sync catches it)
Moves to wiki/concepts/architecture/chunking-strategy.md (canonical)
vlad-wiki page becomes stub linking to canonical
```

### Deviation Flow

```
Plan says: "Use Chroma for vector DB"
         │
         ▼
Phase 0 research → pgvector is better (same DB, zero cost)
         │
         ▼
/sync prompts: "Implementation differs from plan. Capture deviation? [y/n]"
         │
         ▼
Writes to: roadmap/phases/phase-0-research/deviations.md
         │
         ▼
Next time /phase 1 exec runs → boot prompt includes:
  "Phase 0 deviation: pgvector chosen over Chroma.
   Assume pgvector, not a separate vector service."
```

---

## 19. Roadmap Dashboard (Engineering Section)

### Purpose

An internal engineering dashboard in the React app (`app/`) that provides real-time visibility into project state. Modeled on the ops-platform style from aldc-shipyard. Accessible at `/roadmap` in the app.

### Main View: Phase Progress Graph

An interactive graph showing all 11 phases as nodes with connections showing dependencies. Each node displays:
- Phase number and name
- Status (color-coded: gray=not started, blue=planning, yellow=in progress, green=complete)
- Completion percentage (ticket progress bar)
- Who's assigned (engineer avatars/initials)

```
┌──────────────────────────────────────────────────────────────────┐
│  NeuroSpect Roadmap                                  [Timeline] │
│                                                                  │
│  Track A: Coaching                                               │
│                                                                  │
│  ┌────┐   ┌────┐   ┌────┐   ┌────┐   ┌────┐   ┌────┐   ┌────┐ │
...
│                   NULL TEST GATE                                 │
│                                                                  │
│  P = Paul  V = Vlad  ██ = complete  ░░ = in progress            │
└──────────────────────────────────────────────────────────────────┘
```

### Drilldown: Feature Sub-Graph

When a phase node is clicked, a sub-graph expands showing each feature/ticket as a node with dependencies:

```
┌──────────────────────────────────────────────────────────────────┐
│  Phase 1: Knowledge Base & RAG MVP                    [← Back]  │
│                                                                  │
│  ┌─────────────┐     ┌──────────────┐     ┌──────────────┐     │
│  │ NEU-001     │     │ NEU-002      │     │ NEU-004      │     │
│  │ Setup       │────▶│ Embedding    │────▶│ Wiki         │     │
│  │ pgvector    │     │ model eval   │     │ ingestion    │     │
│  │ ██ Done     │     │ ██ Done      │     │ ░░ Progress  │     │
│  │ Vlad        │     │ Vlad         │     │ Vlad         │     │
│  └─────────────┘     └──────────────┘     └──────┬───────┘     │
│                                                   │              │
│                                                   ▼              │
│  ┌─────────────┐     ┌──────────────┐     ┌──────────────┐     │
│  │ NEU-003     │     │ NEU-005      │     │ NEU-006      │     │
│  │ Chunking    │────▶│ RAG          │────▶│ Chat API     │     │
...
│                                            └──────────────┘     │
│                                                                  │
│  Progress: 3/8 tickets done (37%)                                │
│  Blockers: None                                                  │
│  Deviations: 1 (pgvector chosen over Chroma)                    │
└──────────────────────────────────────────────────────────────────┘
```

### Technical Implementation

**Backend (`api/app/routers/roadmap.py`):**

| Endpoint | Method | Purpose |
|---|---|---|
| `GET /api/roadmap/phases` | GET | All phases with status, assignments, ticket counts, Linear links |
| `GET /api/roadmap/phases/{n}` | GET | Phase detail with tickets, deviations, dependencies |
| `GET /api/roadmap/phases/{n}/tickets` | GET | All tickets for a phase with status, assignee, and Linear URL |
| `GET /api/roadmap/status` | GET | Dashboard summary (overall progress, blockers, current focus) |
| `POST /api/roadmap/sync` | POST | Trigger a sync (same logic as `/sync` skill, exposed as API) |
| `GET /api/roadmap/linear/webhook` | POST | Linear webhook receiver for real-time status updates |

**Linear is the source of truth for ticket status.** The dashboard reads from Linear, not from local files. The flow:

```
Linear (source of truth)
    │
    ├──→ Linear webhook ──→ api/roadmap/linear/webhook
    │                           │
    │                           ▼
    │                    Update roadmap/status.md
    │                    Update personal wiki phase READMEs
    │                    Regenerate boot prompts for affected phases
    │
    ├──→ Dashboard API ──→ GET /api/roadmap/phases
    │                           │
    │                           ▼
    │                    React dashboard (real-time)
    │
    └──→ /sync skill ──→ Bidirectional reconciliation
                              (git state → Linear, Linear → roadmap files)
```

**Every item in the dashboard links to Linear:**
- Phase nodes link to the Linear project/cycle view filtered to that phase
- Ticket nodes link directly to the Linear issue URL (`https://linear.app/neurollm/issue/NEU-042`)
- Assignee badges link to the engineer's Linear profile
- Blocker indicators link to the blocking issue in Linear
- The "Open in Linear" button on every phase and ticket drilldown

**Linear webhook integration:**
- Register a webhook in Linear that fires on issue status changes, assignments, and label changes
- The webhook handler (`POST /api/roadmap/linear/webhook`) updates:
  1. `roadmap/status.md` — phase completion percentages
  2. `roadmap/phases/phase-N/README.md` — ticket counts in frontmatter
  3. Personal wiki phase READMEs — synced status fields
  4. Triggers boot prompt regeneration for affected phases
- This means the dashboard is **always current** — not just when `/sync` runs

**Fallback if webhook is unavailable:** The dashboard API polls Linear every 60 seconds. `/sync` does a full bidirectional reconciliation on demand.

**Linear data model mapping:**

| Linear Concept | NeuroSpect Mapping |
|---|---|
| Workspace | `NeuroSpect` |
| Project | `NeuroSpect Platform` |
| Cycle | One per phase (e.g., "Phase 1: RAG MVP") |
| Issue | Individual ticket (NEU-042) |
| Label | `phase-0` through `phase-10`, `frontend`, `backend`, `rag`, etc. |
| Status | `Backlog` → `In Progress` → `In Review` → `Done` |
| Assignee | `vlad` or `paul` |
| Priority | `Urgent` (P0), `High` (P1), `Medium` (P2), `Low` (P3) |

**Dashboard ticket card fields (all linked to Linear):**

```
┌──────────────────────────────────────────┐
│ NEU-042 ← clickable, opens Linear issue  │
│ RAG retrieval pipeline                    │
│                                           │
│ Status: ░░░░░░░░ In Progress              │
│ Assignee: Vlad    Priority: P0            │
│ Phase: 1          Cycle: RAG MVP          │
│ Branch: feat/NEU-042-rag-retrieval        │
│ PR: #15 (open) ← links to GitHub PR      │
│ Blocked by: NEU-041 ← links to Linear    │
│                                           │
│ [Open in Linear]  [View PR]  [View Wiki]  │
└──────────────────────────────────────────┘
```

**Frontend (`app/src/components/roadmap/`):**

| Component | Library | Purpose |
|---|---|---|
| `PhaseGraph.tsx` | React Flow | Interactive directed graph of phases. Each node links to Linear cycle. |
| `PhaseNode.tsx` | Custom + shadcn | Clickable phase node. Status, progress bar, assignee avatars. Click → drilldown. |
| `FeatureSubGraph.tsx` | React Flow | Drilldown: tickets as nodes with dependency edges. Each node links to Linear issue. |
| `TicketNode.tsx` | Custom + shadcn | Ticket card with Linear link, GitHub PR link, assignee, status badge, branch name. |
| `StatusTimeline.tsx` | Recharts | Timeline/Gantt view. Phase bars link to Linear cycles. |
| `EngineerActivity.tsx` | Custom | Recent activity feed. Each entry links to the Linear issue or GitHub PR it references. |
| `DeviationsList.tsx` | shadcn Table | Deviations per phase with links to the commits/PRs that caused them. |
| `BlockersBanner.tsx` | shadcn Alert | Blocker issues with direct Linear links for resolution. |
| `LinearLink.tsx` | Custom | Reusable component: renders a clickable link to any Linear entity (issue, cycle, project). |

**Recommended graph library:** React Flow (`reactflow` npm package, ~150KB, MIT). Works with shadcn/Tailwind, supports interactive nodes, zoom, pan, custom node rendering, and edge labels.

**Additional dependencies:** `reactflow`, `@linear/sdk` (Linear API client for real-time data)

### Routing

```
/roadmap              → Main phase graph view (all phases as nodes)
/roadmap/phase/:n     → Phase drilldown with feature sub-graph (tickets as nodes)
/roadmap/timeline     → Timeline view (Gantt-style, phases as bars)
/roadmap/activity     → Engineer activity feed (recent commits, ticket updates)
```

Every route maintains Linear deep links — clicking any ticket, phase, or assignee opens the corresponding Linear entity in a new tab.

### Sync Architecture

The dashboard must always reflect reality. Three sync mechanisms ensure this:

| Mechanism | Trigger | Latency | Direction |
|---|---|---|---|
| **Linear webhook** | Any Linear status change | < 5 seconds | Linear → dashboard + roadmap files |
| **Dashboard API poll** | Page load + 60s interval | < 60 seconds | Linear → dashboard (fallback if webhook missed) |
| **`/sync` skill** | End of session (manual) | On demand | Bidirectional: git ↔ Linear ↔ roadmap files ↔ personal wikis |

The webhook is the primary mechanism. Polling is the fallback. `/sync` is the full reconciliation that also handles git state and wiki content.

### Change Management System

Built into the dashboard — not dependent on Linear. This is a core feature of the workflow platform (see DevFlow OS product concept in ALDC wiki).

**Change Request workflow:**

```
Engineer flags a phase/ticket for change
    │
    ▼
┌───────────────────────────────────────────────┐
│  Change Request Form (in dashboard)           │
│                                                │
│  What to change: [Phase / Ticket / Plan item] │
│  Type: [Refinement / Deviation / New scope]   │
│  Proposed change: [description]                │
│  Reason: [why]                                 │
│  Impact: [which phases/tickets affected]       │
│  Requested by: [engineer name]                 │
└───────────────────┬───────────────────────────┘
                    │
                    ▼
┌───────────────────────────────────────────────┐
│  Pending Changes Queue (visible in dashboard) │
│                                                │
│  ☐ CR-001: Change backtest engine to Polars   │
│    Requested by: Vlad | Phase 7 | 2026-06-15  │
│    Impact: Phase 7, 8 (feature pipeline)      │
│    Status: Under Review                        │
│                                                │
│  ☐ CR-002: Add crypto market support to MVP   │
│    Requested by: Paul | Phase 1 | 2026-06-18  │
│    Impact: Phase 1, 2, 7 (market data)        │
│    Status: Proposed                            │
└───────────────────┬───────────────────────────┘
                    │
          ┌─────────┴─────────┐
...
**Dashboard page: `/roadmap/changes`**

Shows:
- **Pending** — change requests awaiting review (checkbox to accept/reject)
- **Accepted** — changes incorporated into the plan (links to resulting tickets and deviations)
- **Rejected** — declined changes with documented reasoning
- **All** — full audit trail

Each accepted change automatically:
1. Creates a deviation entry in `roadmap/phases/phase-N/deviations.md`
2. **Creates Linear ticket(s)** if implementation work is needed (via Linear API)
3. **Updates existing Linear tickets** if they're affected by the change (adds comment, updates description, or re-labels)
4. Triggers boot prompt regeneration for affected phases
5. Updates the roadmap status dashboard

**Linear sync is always maintained** — the dashboard owns the change request lifecycle, Linear owns the resulting implementation tickets. When Linear ticket status changes (via webhook), the dashboard updates the CR's implementation progress. This separation means if Linear is later replaced by built-in ticket management, only the ticket creation step changes — the CR workflow stays identical.

**Backend:**

| Endpoint | Method | Purpose |
|---|---|---|
| `GET /api/roadmap/changes` | GET | List all change requests (filterable by status, phase, requester) |
| `POST /api/roadmap/changes` | POST | Submit a new change request |
| `PATCH /api/roadmap/changes/{id}` | PATCH | Accept or reject a change request |
| `GET /api/roadmap/changes/{id}` | GET | Change request detail with audit trail |

**Database table:**

```sql
CREATE TYPE change_request_status AS ENUM ('proposed', 'under_review', 'accepted', 'rejected');
CREATE TYPE change_request_type AS ENUM ('refinement', 'deviation', 'new_scope', 'removal');
...
CREATE TABLE change_requests (
    ...
    status          change_request_status NOT NULL DEFAULT 'proposed',
    requested_by    VARCHAR(64) NOT NULL,          -- engineer name
    phase           INTEGER,                        -- affected phase (nullable for cross-phase)
    affected_phases INTEGER[],                      -- all phases impacted
    affected_tickets VARCHAR(64)[],                 -- Linear ticket IDs impacted
    resolution_note TEXT,                           -- reason for accept/reject
    resolved_by     VARCHAR(64),
    resolved_at     TIMESTAMPTZ,
    -- Links created on acceptance
    deviation_id    UUID,                           -- link to deviation entry
    created_tickets VARCHAR(64)[],                  -- Linear tickets created from this CR
...
| Component | Purpose |
|---|---|
| `ChangeRequestForm.tsx` | Submit a new change request (modal or page) |
| `ChangeRequestQueue.tsx` | List of pending changes with accept/reject actions |
| `ChangeRequestDetail.tsx` | Full detail view with audit trail |
| `ChangesBadge.tsx` | Badge on affected phase nodes showing pending change count |
| `ChangesPage.tsx` | `/roadmap/changes` — full change management view |

**Routing update:**

```
/roadmap/changes          → All change requests (filterable)
/roadmap/changes/new      → Submit new change request
/roadmap/changes/:id      → Change request detail
```

Affected phase nodes in the main roadmap graph show a badge (e.g., "2 pending changes") that links to the filtered changes view.

### Phase of Implementation

The roadmap dashboard (including change management) should be built during **Phase 3 (Product MVP)** alongside other internal tooling. It uses the same React + shadcn + Recharts patterns already established. The change request system is dashboard-native (not Linear-dependent) — this is deliberate, as it's a core DevFlow OS feature that eventually replaces external tooling.

---

## 20. Initial GitHub Issues (Top 20)
...
## 22. NeuroCortex — Knowledge Layer Architecture
...
## 23. Patterns to Replicate from Zeus Memory

Zeus Memory (ALDC's production knowledge system, 3.6M+ memories) has battle-tested patterns to reimplement independently in NeuroSpect. We are NOT using zeus-memory code — building from scratch using the same architectural patterns.
...
## 24. `/sync` — Unified Project State Synchronization
...
### Should we pursue the NeuroSpect platform direction?
